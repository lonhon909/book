...
// .babelrc
{
    // @babel/preset-env只会处理语法（如：let/const等），不会处理api（如：includes等），配置项useBuiltIns告诉babel如果处理api，默认false，即不处理；设置为entry，同时在源代码的最上方手动引入 @babel/polyfill 这个库，打包时会将polyfil全部打入；设置为usage，不需要手动引入polyfill，会自动按需引入
